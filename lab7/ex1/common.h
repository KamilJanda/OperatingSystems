...
#define MAX_WAITING_ROOM_SIZE 10
...
struct barberShop
{
    int barberStatus;
    int clientsInQueue;
    struct client* queue[MAX_WAITING_ROOM_SIZE];
...
void add(struct client* cleint,struct barberShop *barberShop)
{
...
struct client* pop(struct barberShop *barberShop)
{
    if (barberShop->clientsInQueue == 0)
        return NULL;

    struct client* result = barberShop->queue[0];
