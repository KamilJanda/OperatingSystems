...
int main(int argc, char *argv[])
{
    ...
    QUEUE_ID = create_queue(publicKey);
    ...
    while (1)
    {
        if (IS_SERVER_ON == 0)
        {
            ...
            if (msgctl(QUEUE_ID, IPC_STAT, &current_state) == -1)
                perror("server: fail of getting current state of queue\n");

            if (current_state.msg_qnum == 0)
                break;
        }

        if (msgrcv(QUEUE_ID, &message, sizeOfmessage, 0, 0) < 0)
        {
...
void close_queue()
{
    ...
    {
        perror("Server: fail to remove queue\n");
    }
    else
    {
        printf("Server: remove queue \n");
    }
}